...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2 class="fw-bold text-dark mb-0">All Complaints</h2>
</div>

<!-- Search & Filter -->
<div class="card-custom mb-4 p-3">
    <form action="{{ route('admin.complaints') }}" method="GET" class="row g-3 live-search-form" data-target=".table-responsive">
        <div class="col-md-6">
            <input type="text" name="search" class="form-control" placeholder="Search Citizen, Dept, or Subject..." value="{{ request('search') }}">
        </div>
        <div class="col-md-3">
    ...
    <table class="table table-hover align-middle">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><span class="badge bg-secondary">{{ $complaint->department->name ?? 'N/A' }}</span></td>
                        ...
                        <div class="modal-dialog modal-lg">
                            <div class="modal-content">
                                <div class="modal-header bg-light">
                                    <h5 class="modal-title">Complaint #{{ $complaint->complaint_id }}</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                </div>
                                <div class="modal-body">
                                    <div class="row">
                                        <div class="col-md-6">
                                            <p><strong>Citizen:</strong> {{ $complaint->user->name }} ({{ $complaint->user->email ?? 'No Email' }})</p>
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            @else
                                                <div class="alert alert-light text-center">No Image</div>
                                            @endif
                                        </div>
                                    </div>
                                    ...
                                    @if($complaint->feedback)
                                        <div class="alert alert-success mt-2">
                                            <strong>Feedback:</strong> {{ $complaint->feedback->rating }} Stars - "{{ $complaint->feedback->comment }}"
                                        </div>
                                    @endif
                                </div>
